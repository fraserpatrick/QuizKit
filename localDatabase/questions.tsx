...
export const createLocalQuestion = async (quizID: number, type: string, text: string, correctAnswer: string, options: string[], feedback: string): Promise<boolean> => {
    const insertSQL = `INSERT INTO question (quizID, type, text, correctAnswer, options, feedback) VALUES (?, ?, ?, ?, ?, ?)`;
    const data = [quizID, type, text, correctAnswer, JSON.stringify(options), feedback];
...
export const updateLocalQuestion = async (questionID: number, text: string, type: string, correctAnswer: string, options: string[], feedback: string): Promise<boolean> => {
    ...
    const data = [type, text, correctAnswer, JSON.stringify(options), feedback, questionID];
